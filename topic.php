<?php
	...
	if (!isset($_GET["z"])) { header("location: ./home.php?topic=fail&reason=not_work"); exit(); }
	...
	if (empty($topic) || mysql_num_rows($query) == 0){ header("location: ./home.php?topic=fail&reason=not_work"); exit(); }
	
	#проверяем не занята ли уже эта работа
	if (!is_null($topic["intStudentId"])){ header("location: ./current_topic.php?z=".$_GET["z"].""); exit(); }	
...
<p>Направление: <b><?php echo $topic["txtDirectionName"]; ?></b></p>
...
<p>Подано заявок на выполнение работы: <b><?php if (!empty($count_appl["max"])) echo $count_appl["max"]; else echo 0; ?></b> </p>
	...
	<?php 
		  if ($_SESSION["userId"] == $topic['intTeacherId']) echo '<input id="submit" type="button" onclick="if (confirm(\'Bы действительно хотите удалить тему?\')) location.href=\'./php/remove_topic.php?z='.$topic["intWorkId"].'\';" value="Удалить тему" /><input id="submit" type="button" onclick="location.href=\'\edit_topic.php?z='.$topic["intWorkId"].'\'" value="Редактировать тему" />'; 
		  else echo '<input id="submit" type="button" onclick="location.href=\'new_message.php?u='.$name["txtLogin"].'&t='.$topic["txtTopic"].'\'" value="Написать сообщение преподавателю" />';
		  if ($_SESSION["statusId"] == 0 && $topic["txtCourse"] == $_SESSION["courseId"] && $topic["intDirectionId"] == $_SESSION["directionId"]) echo '<input id="submit" type="button" onclick="if (confirm(\'Bы действительно хотите подать заявку?\')) location.href=\'./php/apply.php?z='.$topic["intWorkId"].'\';" value="Подать заявку на выполнение работы" />';
	?>	
