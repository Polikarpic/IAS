...
session_start();
	...
	if (intval($_GET['page8']) > 0) $pages = safe_var(intval($_GET['page8']));
	else $pages = 1;
	
	# получаем максимальное число записей на страницу
	$count = mysql_fetch_array(mysql_query("select COUNT(*) as max from tblWork WHERE boolType='1' and intWorkStatus='1' and txtStudentId is not null"));
	...
	LEFT JOIN tblDeadlines dl ON dl.intDeadlinesId = w.intDeadlinesId
	WHERE w.boolType='1' and w.intWorkStatus='1' and w.txtStudentId is not null ORDER BY w.intWorkId DESC LIMIT ".$row_start.", ".$row_limit."");	
...
?>
				...
				while($data = mysql_fetch_array($query))
				{
					...
					echo '<tr>';
					echo '<td>'.strftime('%G',strtotime($data["dateRentingWork1"])).'</td>'; 
...
echo 'Страница: ';
	...
	{		
		...
		{
			echo '<a href="./../../list?page1='.$_GET['page1'].'&page2='.$_GET['page2'].'&page3='.$_GET['page3'].'&page4='.$_GET['page4'].'&page5='.$_GET['page5'].'&page6='.$_GET['page6'].'&page7='.$_GET['page7'].'&page8='.$p.'&c1='.$_GET['c1'].'&c2='.$_GET['c2'].'&c3='.$_GET['c3'].'&c4=2">'.$p.'</a> ';
		}
	}
}
?>	