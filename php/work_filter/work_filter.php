...
session_start();
	...
	if ($_SESSION["statusId"] == 0)
	{
		...
		WHERE w.intDirectionId='".$_SESSION["directionId"]."' and w.txtCourse='".$_SESSION["courseId"]."' and w.boolType='0' and (w.txtStudentId is null or w.intStudentsPerformingWork < w.txtNumber_of_persons) ORDER BY w.intWorkId DESC LIMIT ".$row_start.", ".$row_limit."");	
	}
	else
	{
		#получаем список последних 5 добавленных работ
...
echo 'Страница: ';
	...
	{		
		...
		{
			echo '<a href="'.$_SERVER["PHP_SELF"].'?page1='.$p.'&page2='.$_GET['page2'].'&page3='.$_GET['page3'].'&page4='.$_GET['page4'].'">'.$p.'</a> ';
		}
	}
}
?>	